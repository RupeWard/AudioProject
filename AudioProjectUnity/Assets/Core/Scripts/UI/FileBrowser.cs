...
namespace RJWS.Core.UI
{
    /*
        File browser for selecting files or folders at runtime.
        See http://wiki.unity3d.com/index.php?title=ImprovedFileBrowser
     */
    ...
    public class FileBrowser
    {
        ...
        protected int m_selectedFile
		{
			...
			{
				_selectedFile = value;
				//Debug.Log( "Selected set to " + _selectedFile );  
			}
		}
        ...
        public FileBrowser(Rect screenRect, string name, FinishedCallback callback, string directory = null )
        {
            if (callback == null)
            {
                throw new System.Exception("No callback provided to FileBrowser ctor");
            }
            m_name = name;
            ...
            {
                Debug.Log("Using current directory ");
                SetNewDirectory(Directory.GetCurrentDirectory());
            }
            else
            {
                Debug.Log("Using directory: '"+directory+"'");
        ...
        protected void ReadDirectoryContents()
        {
            ...
            {
                string permissibleFolder = m_currentDirectory;
...
//					Debug.Log( "Split permissible " + permissibleFolder + " into " + m_currentDirectoryParts.Length );
            ...
            if (BrowserType == FileBrowserType.Directory || string.IsNullOrEmpty(SelectionPattern))
            {
            ...
            {
                string[] selectionPatterns = SelectionPattern.Split(' ');
                List<string> filesFound = new List<string>();
                foreach (string selpat in selectionPatterns)
                {
//                    Debug.LogWarning("Looking for "+selpat+" from selectionPattern '"+SelectionPattern+"'" );
                    string[] files = Directory.GetFiles(m_currentDirectory, selpat);
                    filesFound.AddRange(files);
                }
                m_files = filesFound.ToArray();
        ...
        public void OnGUI()
        {
            ...
            int selectedFile = GUILayoutx.SelectionList(
                m_selectedFile,
                m_filesWithImages,
                FileDoubleClickCallback
            );
//			Debug.Log( "Selected " + m_selectedFile + " -> " + selectedFile );
			if (selectedFile > -1)
			{
