...
#region Scene singletons
		...
		where GameObjectType : MonoBehaviour
	{
		...
		public static GameObjectType Instance
		{
			get
			{
				...
				{
					string className = typeof( GameObjectType ).ToString( );
					int indexOfDot = className.LastIndexOf( "." );
					if (indexOfDot != -1)
					{
						className = className.Substring( indexOfDot + 1 );
					}
                    string instanceName = "The" + className;
	...
	public class SingletonSceneLifetimeLazy<GameObjectType> : MonoBehaviour
		where GameObjectType : MonoBehaviour
	{
		...
		void Awake( )
		{
			...
			{
#if DEBUG_SINGLETONS
				Debug.Log( "Awaking scene lazy singleton of type " + typeof( GameObjectType ).ToString( ) );
#endif //DEBUG_SINGLETONS
				PostAwake( );
			}
		}
		...
		void OnDestroy( )
		{
			...
			if (instance != null)
			{
				...
				{
#if DEBUG_SINGLETONS
					Debug.Log( "Destroying a SingletonSceneLifetimeLazy. Type is " + typeof( GameObjectType ).ToString( ) );
#endif //DEBUG_SINGLETONS
					SingletonHelper<GameObjectType>.HandleOnDestroy( ref instance, ref blockInstanceFetch );
					PostOnDestroy( );
				}
			...
			{
				Debug.LogWarning( "Destroying a SingletonSceneLifetimeLazy when the instance of the singleton is null, this should not happen (unless you change scripts at runtime in the editor)!\n Type is " + typeof( GameObjectType ).ToString( ) );
				PostOnDestroy( );
			}
		}
...
#if DEBUG_SINGLETONS
						Debug.Log( "Created a singleton with a scene lifetime of type " + typeof( GameObjectType ).ToString( ) );
#endif //DEBUG_SINGLETONS
					}
					else
					{
						Debug.LogError( "Unable to create scene singleton of type " + typeof( GameObjectType ).ToString( ) );
					}
				}
...
#endregion Application Singletons
